...
// Copyright (c) 2016-2018 The Soyuz3 developers
...
public:
    ...
    bool WriteZSYZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSYZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSYZSeed_deprecated(uint256& seed);
    bool EraseZSYZSeed();
    bool EraseZSYZSeed_deprecated();

    bool WriteZSYZCount(const uint32_t& nCount);
    bool ReadZSYZCount(uint32_t& nCount);
