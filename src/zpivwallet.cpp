...
CzSYZWallet::CzSYZWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSYZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSYZSeed_deprecated()) {
                    LogPrintf("%s: Updated zSYZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzSYZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSYZCount(nCountLastUsed);
    else if (!walletdb.ReadZSYZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSYZWallet::Lock()
{
    seedMaster = 0;
}

void CzSYZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSYZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSYZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSYZWallet::LoadMintPoolFromDB()
{
...
void CzSYZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSYZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSYZWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSYZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSYZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzSYZWallet::SeedToZSYZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSYZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSYZWallet::GenerateDeterministicZSYZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSYZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSYZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
