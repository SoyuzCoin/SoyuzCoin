...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 SYZ at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSYZ send
enum ZerocoinSpendStatus {
    ZSYZ_SPEND_OKAY = 0,                            // No error
    ZSYZ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSYZ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSYZ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSYZ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSYZ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSYZ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSYZ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSYZ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSYZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSYZ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSYZ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSYZ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSYZ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSYZ_SPENT_USED_ZSYZ = 14,                      // Coin has already been spend
    ZSYZ_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSYZ_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSYZOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzSYZWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSYZTracker> zpivTracker;
    ...
    void setZWallet(CzSYZWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzSYZTracker>(new CzSYZTracker(strWalletFile));
    }

    CzSYZWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSYZ reset */
    boost::signals2::signal<void()> NotifyzSYZReset;
